...
const sql = `
  SELECT athlete.athlete_id, element.element_name, element.element_id
  FROM element
  ...
  WHERE athlete.athlete_id = ? AND element.element_class_id = ?; 
`;
...
async function getMetrics() {
  const metrics = await executeQuery({ query: sql, values: [1, 2] });
  console.log(metrics);
