...
export default async function handler(req, res) {
  ...
  const { athlete0Id, date } = queryParams;
  const today = date;
  const backDate = date;
  ...
  if (req.method === "GET") {
    const metrics = await getHealthcare(athlete0Id);
    console.log({ metrics });
    res.status(200).json({
...
async function getHealthcare(athlete0Id, backDate, today) {
  const sql = `
  ...
  WHERE athlete.unique_identifier = 'auth0|62fa26f0e578d985fe41dce9' AND healthcare_log.date BETWEEN ? AND ?
